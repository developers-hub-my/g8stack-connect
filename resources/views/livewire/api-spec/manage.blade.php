    ...
    <form wire:submit="save" class="space-y-8">
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <flux:input wire:model="slug" label="Slug" placeholder="Auto-generated from name" description="URL path: /api/connect/{slug}" />
            </div>
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-6 space-y-4">
            ...
            @if(empty($availableTables) && !$dataSourceId)
                <p class="text-sm text-zinc-500 dark:text-zinc-400 py-4 text-center">Select a data source first to see available tables.</p>
            @elseif(empty($availableTables) && $dataSourceId)
                <p class="text-sm text-zinc-500 dark:text-zinc-400 py-4 text-center">No introspected tables found. Try re-introspecting the data source.</p>
            @endif
            ...
            <div>
                @if($isEditing && $apiSpec)
                    @if($apiSpec->status->value === 'deployed')
                        <flux:button variant="danger" wire:click="undeploy" type="button" icon="power-off">
                            Undeploy
                        </flux:button>
                    @else
                        <flux:button variant="filled" color="emerald" wire:click="deploy" type="button" icon="rocket">
                            Deploy
                        </flux:button>
                    @endif
                @endif
