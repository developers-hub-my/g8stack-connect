<div>
    <div class="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex gap-2">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="Search data sources..." icon="search" class="w-64" />
            <flux:select wire:model.live="statusFilter" class="w-48">
                <option value="">All Statuses</option>
                @foreach(\App\Enums\ConnectionStatus::cases() as $status)
                    ...
                    <table class="relative min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800">
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-zinc-900 dark:text-zinc-100">Type</th>
                            ...
                            @forelse ($dataSources as $source)
                                <tr>
                                    ...
                                    <td class="px-3 py-4 text-sm">
                                        <flux:badge size="sm" :color="match($source->status->value) { 'connected' => 'green', 'introspected' => 'blue', 'failed' => 'red', default => 'zinc' }">
                                            {{ $source->status->label() }}
                                        </flux:badge>
                                    </td>
                                    <td class="px-3 py-4 text-sm text-zinc-700 dark:text-zinc-300">
                                    ...
                                    <td class="py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                                        <flux:button variant="ghost" size="sm" :href="route('data-sources.show', ['uuid' => $source->uuid])" wire:navigate>
                                            View
                                        </flux:button>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td colspan="5" class="py-12 text-center">
                                        <x-empty-state
                                            ...
                                            description="Get started by connecting a new data source."
                                        />
                                    </td>
                                </tr>
                            @endforelse
