...
| `ApiDraft` | Generated OpenAPI spec awaiting push to G8Stack | v0.1 |
| `ConnectionAudit` | Every connect/introspect/preview action logged | v0.1 |
| `DraftField` | Per-field config (exposed, excluded, renamed, PII-flagged) | v0.2 |
| `DraftVersion` | Versioned drafts — regenerate creates new version, never overwrites | v0.2 |

### Credential Handling — CRITICAL

**Never store raw credentials.** All data source credentials must be:
- Encrypted at rest using Laravel's `encrypt()`/`decrypt()`
...
// DON'T — never store plain
$dataSource->credentials = $validated['credentials'];
```
...
- Submit drafts with metadata: source type, generated by, timestamp
...
{
    public function pushDraft(ApiDraft $draft): PushResult;
    public function getApprovalStatus(string $draftId): DraftStatus;
}
```
...
| `draft.generate` | Generate OpenAPI draft |
| `draft.push` | Push draft to G8Stack |
| `draft.view` | View generated drafts |
...
- **Credentials columns**: Always cast with `encrypted` cast
...
├── Enums/              # DataSourceType, DraftStatus, ConnectionStatus, WizardMode
├── Livewire/           # Livewire components
│   ├── DataSource/     # Connect, Introspect, Preview wizards
│   ├── Draft/          # Draft review, configure, push
...
│   ├── DraftGenerator/     # OpenAPI spec generation
│   │   ├── CrudDraftGenerator.php    # Simple Mode (v0.1)
│   │   ├── GuidedDraftGenerator.php  # Guided Mode (v0.2)
│   │   └── SqlDraftGenerator.php     # Advanced Mode (v0.4)
│   └── G8StackService.php  # Push drafts to G8Stack (v0.5)
